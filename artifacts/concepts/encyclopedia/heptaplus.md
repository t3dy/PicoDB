...
- Artifact ID: `encyclopedia_concept_heptaplus`
...
- Primary Pico loci: *Heptaplus*
- Guide scholars: Black, Busi, Wirszubski

## Definition

Pico's sevenfold Genesis commentary and the central text for biblical hermeneutics, cosmic order, and post-crisis recoding. In PicoDB this concept is not a free-floating keyword. It is a controlled research object: every use should be attached to passages, works, source traditions, guide scholars, and a confidence level. The concept should therefore be read as both a term in Pico's intellectual vocabulary and a modern historiographical category that can clarify or distort the evidence.

## Place in Pico's Works

The concept belongs especially to *Heptaplus*. Future close reading should identify where the term appears explicitly, where it operates implicitly, and where later scholarship imposes the label for analytic convenience. Pico's writings often move across genres: public disputation, commentary, apology, biblical exegesis, devotional writing, polemic, and poetic theology. A concept such as Heptaplus may therefore change function as it moves from one genre to another.
...
The initial guide scholars for this entry are Black, Busi, Wirszubski. Their role is methodological as well as informational. PicoDB should ask what each scholar teaches us to notice: source language, textual transmission, scholastic terminology, Kabbalistic mediation, reception history, edition status, genre, or philosophical architecture. No concept entry should rely on one label when the evidence requires a layered account.
...
- Which passages in Pico explicitly name or enact Heptaplus?
